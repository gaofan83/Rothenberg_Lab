library(sleuth)
s2c<-read.table("sample_sheet.txt",header=T,stringsAsFactors=FALSE)
...
so1 <- sleuth_fit(so1, ~group, 'reduced')
so1 <- sleuth_fit(so1, ~drug + group, 'full')
...
write.table(sleuth_table_gene, file="sleuth_table_gene_deg_drug.txt", quote=F,row.names=F,sep="\t")
write.table(sleuth_table_tx, file="sleuth_table_tx_deg_drug.txt", quote=F,row.names=F,sep="\t")
